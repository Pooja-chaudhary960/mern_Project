...
const OrderCard = ({ order }) => {
  return (
    <div className="border border-gray-300 bg-white dark:bg-gray-800 dark:border-gray-900 dark:text-white rounded-lg shadow-md overflow-hidden">
      <div className="py-4 px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {order.orderItems.map((item, index) => (
            <div key={index} className="border border-gray-200 dark:border-gray-700 bg-gray-50 rounded-md px-4 py-2">
              <div className="flex items-center gap-4">
                {item.product?.imageUrls.length > 0 ? (
                  <Image
                    ...
                    width={100}
                    height={100}
                    className="h-14 w-14 object-cover rounded-md border-gray-200"
                  />
                ) : (
                  <FaImage className="h-18 w-18 text-gray-300" />
                )}
                <div>
                  <h4 className="text-lg font-medium text-gray-800 dark:text-white">{item.product?.name}</h4>
                  <p className="text-gray-600 dark:text-green-300">
                    Total: Rs. {item.product.price} X {item.quantity}
                  </p>
                </div>
              </div>
            </div>
